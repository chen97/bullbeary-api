...
def myResponse(request):
    Message = "Omg"
    return JsonResponse({'message': f'{Message} Amazing.'}, status=status.HTTP_200_OK)


def get_details(request, stock_code):
    return JsonResponse({'Code': f'{stock_code} Amazing.'}, status=status.HTTP_200_OK)
